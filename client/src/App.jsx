...
export default function App() {
  const {
    messages, essayStructure, isLoading, isSpeaking,
    error, currentStage, stages, sendMessage, startSession, stopSpeaking,
  } = useConversation()
  ...
  const handleRestart = async () => {
    ...
    setTimeout(async () => { setSessionStarted(true); await startSession() }, 100)
  }
    ...
    return (
      <div className="min-h-screen bg-surface-base flex items-center justify-center px-4">
        ...
        <div className="bg-surface-card rounded-3xl border border-border-subtle shadow-2xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-[13px] text-ivy-purple-dk font-medium mb-4 tracking-wide uppercase">AI Essay Brainstormer</p>
          <p className="text-[14px] text-text-secondary leading-relaxed mb-8">
            Ivy, your AI coach, will interview you through a voice conversation, learn your story, and generate a personalised 350-word essay structure — entirely without human intervention.
          </p>
          ...
          <button onClick={handleStart} className="w-full bg-ivy-purple text-white font-medium rounded-xl py-3.5 text-[15px] hover:bg-ivy-purple-md transition-colors">
            Begin interview →
          </button>
          <p className="text-[12px] text-text-muted mt-4">Voice + text supported · Takes ~8 minutes</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-screen bg-surface-base overflow-hidden">
      ...
      <header className="bg-surface-card border-b border-border-subtle px-6 py-3.5 flex items-center justify-between shrink-0 z-10">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-serif text-lg text-ivy-purple-dk">HelloIvy</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar stages={stages} currentStage={currentStage} essayReady={!!essayStructure} />

        <div className="flex flex-col flex-1 overflow-hidden">
          <ChatWindow messages={messages} isLoading={isLoading} />

          {error && (
            <div className="mx-6 mb-2 text-[13px] text-red-400 bg-red-950/40 border border-red-800/40 rounded-xl px-4 py-2">⚠ {error}</div>
          )}
              ...
              <div className="flex items-end gap-3">
                <div id="tour-mic">
                  <VoiceButton
                    ...
                    isSpeaking={isSpeaking} isLoading={isLoading}
                    onToggle={toggleRecording} onStopSpeaking={stopSpeaking}
